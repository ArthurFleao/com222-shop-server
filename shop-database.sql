...
--
-- Estrutura stand-in para view `view_authors`
...
--
-- Estrutura stand-in para view `view_bookinfos`
-- (Veja abaixo para a visão atual)
--
CREATE TABLE `view_bookinfos` (
`CategoryID` int(4)
...
--
-- Estrutura stand-in para view `view_categories`
-- (Veja abaixo para a visão atual)
--
...
--
-- Estrutura para view `view_authors`
--
DROP TABLE IF EXISTS `view_authors`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_authors`  AS  select `ba`.`AuthorID` AS `AuthorID`,`ba`.`nameF` AS `nameF`,`ba`.`nameL` AS `nameL`,`bab`.`ISBN` AS `ISBN` from (`bookauthors` `ba` join `bookauthorsbooks` `bab` on((`ba`.`AuthorID` = `bab`.`AuthorID`))) ;

-- --------------------------------------------------------

--
-- Estrutura para view `view_bookinfos`
--
DROP TABLE IF EXISTS `view_bookinfos`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_bookinfos`  AS  select `bc`.`CategoryID` AS `CategoryID`,`bc`.`CategoryName` AS `CategoryName`,`bs`.`ISBN` AS `ISBN`,`bs`.`title` AS `title`,`bs`.`description` AS `description`,`bs`.`price` AS `price`,`bs`.`publisher` AS `publisher`,`bs`.`pubdate` AS `pubdate`,`bs`.`edition` AS `edition`,`bs`.`pages` AS `pages` from ((`bookcategories` `bc` join `bookcategoriesbooks` `bcb` on((`bc`.`CategoryID` = `bcb`.`CategoryID`))) join `bookdescriptions` `bs` on((`bcb`.`ISBN` = `bs`.`ISBN`))) ;

-- --------------------------------------------------------

--
-- Estrutura para view `view_categories`
--
DROP TABLE IF EXISTS `view_categories`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_categories`  AS  select `bc`.`CategoryID` AS `CategoryID`,`bc`.`CategoryName` AS `CategoryName`,`bcb`.`ISBN` AS `ISBN` from (`bookcategories` `bc` join `bookcategoriesbooks` `bcb` on((`bc`.`CategoryID` = `bcb`.`CategoryID`))) ;
